import numpy as np
import os
from ..dist import truncated_gumbel
...
if reload_readout:
    ...
    np.savez_compressed('lookup_truncated_gumbel_scale_noisy_readout.npz', mode=xrange_readout, scale=srange_readout,
                        truncscale=data_readout[0])
else:
    data_readout = np.load(os.path.join('lookup_truncated_gumbel_scale_readout.npz'))
...
if reload_report:
    ...
    np.savez_compressed('lookup_truncated_gumbel_scale_noisy_report.npz', mode=xrange_report, scale=srange_report,
                        truncscale=data_report[0])
else:
    data_report = np.load(os.path.join('lookup_truncated_gumbel_scale_report.npz'))
