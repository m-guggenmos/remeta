...
class ParameterSet(ReprMixin):
    def __init__(self, parameters, names, constraints=None):
        ...
        self.base_len = [len(parameters[p]) if isinstance(parameters[p], list) else 1 for p in names]
        self.names = sum([[f'{p}_{i}' for i in range(len(parameters[p]))] if isinstance(parameters[p], list)
...
class Data(ReprMixin):
    ...
    def preproc(self):
        ...
        if np.max(np.abs(self.stimuli)) > 1:
